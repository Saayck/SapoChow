...
async def _setup_exam(client, headers):
    ...
    for i in range(5):
        ...
        qid = q_resp.json()["id"]
        await client.post(f"/api/questions/{qid}/approve", headers=headers)
...
async def test_generate_versions(client: AsyncClient, auth_headers: dict):
    exam_id = await _setup_exam(client, auth_headers)
    resp = await client.post(f"/api/exams/{exam_id}/versions", json={}, headers=auth_headers)
    assert resp.status_code == 201
    assert len(resp.json()) == 2


async def test_answer_key_has_correct_structure(client: AsyncClient, auth_headers: dict):
    exam_id = await _setup_exam(client, auth_headers)
    ...
    # Debe tener 5 entradas, una por pregunta
    assert len(answer_key) == 5
    # Cada valor debe ser una letra válida
    for letter in answer_key.values():
        assert letter in ["A", "B", "C", "D", "E"]


async def test_two_versions_have_independent_answer_keys(client: AsyncClient, auth_headers: dict):
    exam_id = await _setup_exam(client, auth_headers)
    await client.post(f"/api/exams/{exam_id}/versions", json={}, headers=auth_headers)
    ...
    # Ambas claves deben tener el mismo conjunto de posiciones
