  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Temas</h1>
          <p className="text-gray-400 text-sm mt-1">Organiza las preguntas por tema</p>
        </div>
      ...
      {topics?.length === 0 ? (
        ...
        <div className="grid gap-3">
          {topics?.map((topic) => (
            <div
              key={topic.id}
              className="group bg-white border border-gray-200/80 rounded-2xl px-5 py-4 flex items-start justify-between hover:border-gray-300 hover:shadow-card-hover transition-all duration-200"
            >
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center shadow-sm shadow-blue-500/10 shrink-0">
                  <BookOpen size={16} className="text-white" />
                </div>
                <div>
                  <p className="font-semibold text-gray-900">{topic.name}</p>
                  {topic.description && (
                    <p className="text-sm text-gray-400 mt-0.5 line-clamp-1">{topic.description}</p>
                  )}
                </div>
              </div>
              <span className="ml-auto mr-3 shrink-0 inline-flex items-center gap-1 text-xs font-medium text-primary-600 bg-primary-50/80 px-2.5 py-1 rounded-lg border border-primary-100">
                {topic.question_count} pregunta{topic.question_count !== 1 ? 's' : ''}
              </span>
              <div className="flex gap-1 shrink-0 sm:opacity-0 sm:group-hover:opacity-100 transition-all duration-200">
                <button
      ...
      >
        <p className="text-sm text-gray-500 mb-4">
          ¿Estás seguro de que deseas eliminar <strong className="text-gray-700">{deleting?.name}</strong>? Esta acción no se puede deshacer.
        </p>
